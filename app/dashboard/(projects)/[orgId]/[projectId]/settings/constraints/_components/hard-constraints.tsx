  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-1">
        <h2 className="font-medium text-md">Hard Constraints</h2>
      </div>
          ...
          {NUMBER_CONSTRAINTS.map((constraint) => (
            <div key={constraint.id} className="flex items-start justify-between gap-4">
              ...
              <Input
                ...
                type="number"
                min={constraint.min}
                value={hardConstraints[constraint.key] ?? constraint.min} // <- fixed
                onChange={(e) =>
                  onUpdate({ [constraint.key]: parseInt(e.target.value) || constraint.min })
                }
                className="w-20"
              />
            </div>
