...
import { ChevronDown, Plus, GripVertical, Copy, Trash2 } from 'lucide-react';
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              <th className="border border-t-0 border-l-0 px-4 py-2 text-left font-semibold w-24 sticky left-0 bg-gray-50 z-10">Week</th>
              <th className="border border-t-0 px-4 py-2 text-left font-semibold w-24 sticky left-24 bg-gray-50 z-10">Day</th>
              {columns.map(col => (
            ...
            {gridData.map(({ week, days }) => (
              <React.Fragment key={week.id}>
                {days.length > 0 ? (
                  days.map((dayData, dayIndex) => (
                    ...
                    >
                      {dayIndex === 0 && (
                        <td
                          rowSpan={days.length}
                          className={`border border-l-0 px-4 py-2 font-medium bg-gray-50 align-center sticky left-0 z-10 group/week`}
                      ...
                      <td 
                        className="border px-4 py-2 sticky left-24 bg-white hover:bg-gray-50 z-10 align-center group/day"
                        ...
                        if (period) {
                            ...
                            <td key={col} className={`border p-0 ${typeColors[period.type]} hover:brightness-105`}>
                              ...
                              >
                                <PopoverTrigger asChild>
                                  <button className="w-full h-10 hover:opacity-80 transition-opacity">
                                  </button>
                                </PopoverTrigger>
                                  ...
                                  <div className="space-y-1">
                                    {(['Registration', 'Lesson', 'Break', 'Lunch', 'Twilight'] as DayType[]).map(type => (
                                      <button
                                        ...
                                        className={`w-full px-3 py-2 text-left text-sm rounded transition-colors ${typeColors[type]} ${period.type === type ? 'ring-2 ring-gray-900' : 'hover:opacity-80'}`}
                                      >
                                        {type}
                                      </button>
                                    ))}
                                    ...
                                    <button
                                      ...
                                      className="w-full px-3 py-2 flex items-center gap-2 text-left text-sm rounded hover:bg-red-100 text-red-600 transition-colors"
                                    >
                          ...
                          return (
                            <td key={col} className="border p-0">
                              ...
                              >
                                <PopoverTrigger asChild>
                                  <button className="w-full h-10 hover:bg-gray-100 transition-colors">
                                  ...
                                  <div className="space-y-1">
                                    {(['Registration', 'Lesson', 'Break', 'Lunch', 'Twilight'] as DayType[]).map(type => (
                                      <button
                                        ...
                                        className={`w-full px-3 py-2 text-left text-sm rounded hover:opacity-80 transition-colors ${typeColors[type]}`}
                                      >
                                        {type}
                                      </button>
                                    ))}
                  ...
                  <tr>
                    <td 
                      className={`border border-l-0 px-4 py-2 font-medium bg-gray-50 sticky left-0 z-10 group/week`}
                      onMouseEnter={() => setHoveredWeekId(week.id)}
                      onMouseLeave={() => setHoveredWeekId(null)}
                    >
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 z-50">
                    ...
                    <td colSpan={26} className="border px-4 py-2 text-center text-gray-400 text-sm">
